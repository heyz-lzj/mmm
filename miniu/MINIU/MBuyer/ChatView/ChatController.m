...
@interface ChatController ()<UITableViewDataSource, UITableViewDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate, SRRefreshDelegate, IChatManagerDelegate, DXChatBarMoreViewDelegate, DXMessageToolBarDelegate, LocationViewDelegate, IDeviceManagerDelegate>
{
...
- (instancetype)initWithChatter:(NSString *)chatter
{
    ...
    if (self) {
        ...
        _isPlayingAudio = NO;
        //根据接收者的username获取当前会话的conversation
...
- (void) selectAction
{
    [SMActionSheet showSheetWithTitle:@"请选择" buttonTitles:@[@"TA的订单", @"创建订单" ,@"切换客服" ,@"清空消息记录"] redButtonIndex:-1 completionBlock:^(NSUInteger buttonIndex, SMActionSheet *actionSheet) {
...
//                orderListVC.withUserId = self.chatterEntity.userId;
//                [self.navigationController pushViewController:orderListVC animated:YES];
                
                OrderTableViewController *orderTVC = [[OrderTableViewController alloc]init];
                orderTVC.withUserId =  _chatterEntity.userId;
                [self.navigationController pushViewController:orderTVC animated:YES];
                
            }
                break;
            case 1: {
                CreateOrderViewController *createOrderVC = [ CreateOrderViewController new];
                [createOrderVC createOrderWithApplyId:self.chatterEntity.userId addCallBackBlock:^(OrderEntity *createOrder) {
                    
                }];
                
                
                UINavigationController *nav = [[UINavigationController alloc] initWithRootViewController:createOrderVC];
                [self presentViewController:nav animated:YES completion:nil];
            }
                break;
            case 2: {
                HXServiceListViewController *hxserveiceVC = [[HXServiceListViewController alloc] init];
                hxserveiceVC.changeUId = self.chatterEntity.userId;
                [self.navigationController pushViewController:hxserveiceVC animated:YES];
            }
                break;
            case 3: {
...
{
    NSLog(@"当前的聊天界面 datasource 的个数为:%d",self.dataSource.count);
        ...
        if (model.extMessageType == extMessageType_Order || model.extMessageType == extMessageType_Order_Address || model.extMessageType == extMessageType_Order_Logistics || model.extMessageType == extMessageType_Order_Refund) {
            ...
            [cell addCallBackBlock:^(MessageModel *messageModel) {
                ...
                if (messageModel.extMessageType == extMessageType_Order || messageModel.extMessageType == extMessageType_Order_Address || messageModel.extMessageType == extMessageType_Order_Refund) {
                    ...
                    [[logicShareInstance getOrderManager]getOrderDetailsWithOrderNo:messageModel.order.orderNo success:^(id responseObject) {
...
- (MessageModel *)disposeMessageWithMessageModel:(MessageModel *)model
{
    UserEntity *user = [[UserEntity alloc] init];
    // 对方
    if ([self.chatterName isEqualToString:model.username]) {
...
- (void)routerEventWithName:(NSString *)eventName userInfo:(NSDictionary *)userInfo
{
    ...
    if ([eventName isEqualToString:kRouterEventTextURLTapEventName]) {
        ...
        // 米妞买手端，如果是点击自己头像的话则不进行操作
        if (model.userId == self.myEntity.userId) {
            return;
        }
...
-(void)addChatDataToMessage:(EMMessage *)message
{
    __weak ChatController *weakSelf = self;
    dispatch_async(_messageQueue, ^{
        NSArray *messages = [weakSelf addChatToMessage:message];//加入时间消息
...
//        }
        ...
        [SMActionSheet showSheetWithTitle:@"确定清空与该用户的所有聊天记录吗?" buttonTitles:@[@"确定"] redButtonIndex:0 completionBlock:^(NSUInteger buttonIndex, SMActionSheet *actionSheet) {
            if (buttonIndex == 0) {
...
//        [self addChatDataToMessage:tempMessage];
//    }
    EMMessage *tempMessage = [ChatSendHelper sendTextMessageWithString:textMessage toUsername:_conversation.chatter isChatGroup:NO requireEncryption:NO];
    [self addChatDataToMessage:tempMessage];
}

-(void)sendImageMessage:(UIImage *)imageMessage
{
    EMMessage *tempMessage = [ChatSendHelper sendImageMessageWithImage:imageMessage toUsername:_conversation.chatter isChatGroup:NO requireEncryption:NO];
    [self addChatDataToMessage:tempMessage];
}

-(void)sendAudioMessage:(EMChatVoice *)voice
{
    EMMessage *tempMessage = [ChatSendHelper sendVoice:voice toUsername:_conversation.chatter isChatGroup:NO requireEncryption:NO];
    [self addChatDataToMessage:tempMessage];
}

-(void)sendVideoMessage:(EMChatVideo *)video
{
    EMMessage *tempMessage = [ChatSendHelper sendVideo:video toUsername:_conversation.chatter isChatGroup:NO requireEncryption:NO];
    [self addChatDataToMessage:tempMessage];
}
