...
{
    if (_delegate && [self.delegate respondsToSelector:@selector(photoBrowser:deleteCalback:)]) {
        [_delegate photoBrowser:self deleteCalback:self.assetsArray];
    }
...
{
    // 删除图片
    [self.assetsArray removeObjectAtIndex:self.currentPage];
    
    NSInteger assetsCount = [self.assetsArray count];
    
    if (!assetsCount) {
        [self closePhotoBrower];
        return;
    } else if (self.currentPage == assetsCount) {
        self.currentPage --;
    }
...
- (void)show:(BOOL)animated
{
    ...
    if (self.isJkAsset) {
        [_topView addSubview:_delButton];
    } else {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.isJkAsset) {
        JKAssets *jkAsset = [self.assetsArray objectAtIndex:indexPath.row];
        cell.asset = jkAsset.alasset;
    } else {
...
- (void)updateStatus
{
    ...
    ALAsset  *asset;
    if (self.isJkAsset) {
        JKAssets *jkAsset = [self.assetsArray objectAtIndex:_currentPage];
        asset = jkAsset.alasset;
    } else {
        asset = [self.assetsArray objectAtIndex:_currentPage];
    }
...
- (UIView *)topView{
    if (!_topView) {
        ...
        UIImage  *delimg1 = [UIImage imageNamed:@"photowindow_trash"];
        _delButton.frame = CGRectMake(0, 0, delimg1.size.width / 1.5, delimg1.size.height / 1.5);
        [_delButton setBackgroundImage:delimg1 forState:UIControlStateNormal];
        [_delButton addTarget:self action:@selector(photoDidDel) forControlEvents:UIControlEventTouchUpInside];
        _delButton.exclusiveTouch = YES;
        _delButton.right = self.width-10;
        _delButton.centerY = button.centerY;
