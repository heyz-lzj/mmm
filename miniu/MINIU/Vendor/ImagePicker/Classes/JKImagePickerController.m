...
- (void)passSelectedAssets
{
    ...
    for (JKAssets *jka in self.selectedAssetArray) {
        ...
        [[logicShareInstance getAssetManager].assetsLibrary assetForURL:jka.assetPropertyURL
...
- (void)loadAllAssetsForGroups
{
    ...
    self.assetsArray = [NSMutableArray arrayWithArray:[[assets reverseObjectEnumerator] allObjects]];
...
//    
//    NSIndexPath *indexPath = [NSIndexPath indexPathForItem:[self.assetsArray count] inSection:0];
//    
...
- (void)loadAssetsGroupsWithTypes:(NSArray *)types completion:(void (^)(NSArray *assetsGroups))completion
{
    ...
    for (NSNumber *type in types) {
        __weak typeof(self) weakSelf = self;
        [[logicShareInstance getAssetManager].assetsLibrary enumerateGroupsWithTypes:[type unsignedIntegerValue]
...
- (NSArray *)sortAssetsGroups:(NSArray *)assetsGroups typesOrder:(NSArray *)typesOrder
{
    ...
    return [assetsGroups sortedArrayUsingComparator:^NSComparisonResult(ALAssetsGroup *obj1, ALAssetsGroup *obj2) {
        if (obj1.numberOfAssets > obj2.numberOfAssets) {
            return (NSComparisonResult)NSOrderedAscending;
        } else if (obj1.numberOfAssets < obj2.numberOfAssets) {
...
- (void)addAssetsObject:(ALAsset *)alasset
{
...
- (UIToolbar *)toolbar{
    if (!_toolbar) {
        ...
        label.textColor = [UIColor colorWithRed:0.996 green:0.769 blue:0.176 alpha:1];
