...
- (void) setImagesWith:(NSArray *)images
{
    ...
    if (imagesCount) {
        ...
        for (UIImage *image in images) {
            CGRect buttonframe = CGRectMake((65 + 8) * (i % 4) + 15, (65 + 8) * (i / 4) + 86, 65, 65);
        ...
        CGRect buttonframe = CGRectMake((65 + 8) * (i % 3) + 15, (65 + 8) * (i / 3) + 86, 65, 65);
        UIImageView *imageView = [[UIImageView alloc] initWithFrame:buttonframe];
        [imageView setImage:[UIImage imageNamed:@"++++___"]];
        imageView.tag = 8888;
        imageView.clipsToBounds = YES;
...
- (void) setAssetWith:(NSArray *)images
{
    ...
    if (imagesCount) {
        ...
        for (ALAsset *alasset in images) {
            CGRect buttonframe = CGRectMake((65 + 8) * (i % 4) + 15, (65 + 8) * (i / 4) + 8, 65, 65);
            UIImageView *imageView = [[UIImageView alloc] initWithFrame:buttonframe];
        ...
        CGRect buttonframe = CGRectMake((65 + 8) * (i % 3) + 15, (65 + 8) * (i / 3) + 8, 65, 65);
        UIImageView *imageView = [[UIImageView alloc] initWithFrame:buttonframe];
        [imageView setImage:[UIImage imageNamed:@"++++___"]];
        imageView.tag = 8888;
        imageView.clipsToBounds = YES;
...
- (BOOL)validateNumber:(NSString*)number text:(NSString *)text textField:(UITextField *)textField
{
    ...
    if (![text length] && [number isEqualToString:@"."]) {
        [ProgressHUD showError:@"请输入正确的金额!"];
        res = NO;
    }
    ...
    if ([[NSString stringWithFormat:@"%@%@", text, number] integerValue] >= 9999999) {
        [ProgressHUD showError:@"亲,这么大的金额,我们帮不了你了...!"];
        res = NO;
    }
    ...
    if (range.location != NSNotFound) {
        ...
        if ([s length] > 2) {
            [ProgressHUD showError:@"只允许保留小数点后两位!"];
            res = NO;
        }
    }
