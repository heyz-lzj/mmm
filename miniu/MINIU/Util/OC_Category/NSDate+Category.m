...
-(NSString *)formattedTime
{
    ...
    if (!hasAMPM) { //24小时制
        if (hour <= 24 && hour >= 0) {
            dateFormatter = [NSDateFormatter dateFormatterWithFormat:@"HH:mm:ss"];
        }else if (hour < 0 && hour >= -24) {
            dateFormatter = [NSDateFormatter dateFormatterWithFormat:@"昨天HH:mm:ss"];
        }else {
            dateFormatter = [NSDateFormatter dateFormatterWithFormat:@"MM-dd HH:mm:ss"];
        }
    }else {
        if (hour >= 0 && hour <= 6) {
            dateFormatter = [NSDateFormatter dateFormatterWithFormat:@"凌晨hh:mm:ss"];
        }else if (hour > 6 && hour <=11 ) {
            dateFormatter = [NSDateFormatter dateFormatterWithFormat:@"上午hh:mm:ss"];
        }else if (hour > 11 && hour <= 17) {
            dateFormatter = [NSDateFormatter dateFormatterWithFormat:@"下午hh:mm:ss"];
        }else if (hour > 17 && hour <= 24) {
            dateFormatter = [NSDateFormatter dateFormatterWithFormat:@"晚上hh:mm:ss"];
        }else if (hour < 0 && hour >= -24){
            dateFormatter = [NSDateFormatter dateFormatterWithFormat:@"昨天HH:mm:ss"];
        }else  {
            dateFormatter = [NSDateFormatter dateFormatterWithFormat:@"MM-dd HH:mm:ss"];
...
- (BOOL) isEqualToDateIgnoringTimeSSSS:(NSDate *)aDate
{
    ...
    return ([[dateFormatter stringFromDate:self] integerValue] == [[dateFormattera stringFromDate:aDate] integerValue]);
}
...
{
    return [[NSDate dateWithTimeIntervalInMilliSecondSince1970:time] formattedTimeSingle];
}
