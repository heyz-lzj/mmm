...
@property (nonatomic, strong) UIView *lastScreenBGView;
...
#if TARGET_IS_MINIU
@property (nonatomic, strong) ChatToolBar *toobar;
#else
@property (nonatomic, strong) UIToolbar *toobar;  //无用
#endif
...
- (instancetype)initWithRootViewController:(UIViewController *)rootViewController
{
    ...
    if (self) {
#if TARGET_IS_MINIU
        if (![rootViewController isKindOfClass:[ChatViewController class]]) {

            _toobar = [ChatToolBar shareInstance];
...
- (void)viewDidLoad
{
    ...
    UIView *leftView = [[UIView alloc] initWithFrame:CGRectMake(0, 45, 10, kScreen_Height - 45)];
    [self.view addSubview:leftView];
    
    UIPanGestureRecognizer *panRecognizer = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(handlePanGesture:)];
    [leftView addGestureRecognizer:panRecognizer];
}
...
- (UIView *)containerView {
    if (nil == _containerView) {
        ...
        [_containerView addBadgeTip:nil];
    }
...
- (UIImageView *)lastScreenImageView {
    if (nil == _lastScreenImageView) {
        ...
        _lastScreenBGView = [[UIView alloc] initWithFrame:_lastScreenImageView.frame];
        _lastScreenBGView.backgroundColor = [UIColor darkGrayColor];
        _lastScreenBGView.alpha = 0.6;
        [_lastScreenImageView addSubview:_lastScreenBGView];
    }
...
- (NSMutableArray *)screenShots {
    if (nil == _screenShots) {
        _screenShots = [[NSMutableArray alloc]init];//[@[] mutableCopy];
    }
...
- (void)__moveViewWithOffsetX:(CGFloat)x {
    ...
    CGFloat alpha = 0.6 - x / 700;
    _lastScreenBGView.alpha = alpha > 0.6 ? 0.6 : alpha;
//    
//    NSLog(@"%f", alpha);
